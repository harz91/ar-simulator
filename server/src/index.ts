...
app.get('/api/projects/:id/manifest', async (req, res) => {
  try {
    ...
    res.json({
      bakedGlbUrl: project.bakedGlbPath
        ? `https://storage.googleapis.com/${process.env.FIREBASE_STORAGE_BUCKET}/${project.bakedGlbPath}`
        : null,
      steps,
    });
  } catch (error) {
...
app.post('/api/projects/:id/bake', async (req, res) => {
  try {
    ...
    const steps = stepsSnapshot.docs.map(
      (doc) => ({ id: doc.id, ...doc.data() }) as Step,
    );
    ...
    await db
      .collection('projects')
      .doc(id)
      .update({ bakedGlbPath: `projects/${id}/baked.glb` });
